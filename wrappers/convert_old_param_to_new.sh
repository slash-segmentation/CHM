...
{
    echo "Usage:
$0 [<model_folder> [<Nstage>]]
  model_folder    The model folder, defaults to ./temp/
                  (Contains param.mat and MODEL_level#_stage#.mat)
  Nstage          The number of stages used during training, but if not
                  provided can be calculated from the model folder" 1>&2;
  exit 1;
}
...
matlab -nodisplay -singleCompThread -nojvm -r "run_from_shell('convert_old_param_to_new($ARGS);');";
matlab_err=$?;
